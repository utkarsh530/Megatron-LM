...
def padded_swiglu(
    ...
    Mirrors padded_squared_relu but for gated SwiGLU: FC1 output is 2x wide (gate=first half,
    up=second half; megatron SiLU(chunk0)*chunk1), so output is half the input width.
...
def _silu_mul_bounded_kernel(
    ...
    """SiLU(gate) * up on the first n_rows rows only; rows beyond are left untouched.

    Same structure as _swiglu_kernel but WITHOUT the permutation-map check: rows are in
    flat token-major layout ([token*topk + k]) as produced by the vLLM fused-MoE FC1,
    where all rows < valid_tokens*topk are live and everything beyond is garbage that
    downstream kernels never read. n_rows is a device scalar, so the amount of work
    tracks the actual token count inside CUDA graphs — no host-side shape assumptions.
    """
    pid = tl.program_id(0)
